...
#define Terminal1_4 4
#define res_4 5 //res#4 is on portL, rest is on PORTF 
#define Terminal2_4  5
#define Terminal3_4  6
#define Terminal1ADC_4 0b00000100
...
int readPositionADC()
{
	ADMUX |= 1<<ADLAR; // 8-bit ADC from ADCH
...
{ 
	uint8_t temp = 0;
	digitalWrite(&ADMUX,ADLAR,0); // 10-bit ADC 
	ADCSRA |= 1<<ADEN;		//turn on ADC feature
	ADCSRA |= 1<<ADSC;	//start first conversion
	while(ADCSRA&0x40); //start and wait until conversion complete 

	//	lowByte = ((uint8_t)(ADCW & 0xFF));
	
	//temp = map(log(1+ADCW) / log(1+1023),0,1,0,255);
	return ADCW;
} 
...
void padInput() {
	...
	for(int i = 0; i < ROW_COUNT; i++) {                         // for all rows
	  ...
	  for(int j = 0; j < COLUMN_COUNT; j++) {
		...
		//   int temp = readPositionADC();
		
		
		
		
		
		  if (temp <= TOUCH_THRESH) temp = 0;         
		  printPad(temp);              
		  fprintf(&uart_str, " ");                     // for padding
	  }
	  newline();
	}
	newline();
...
void init() {
	...
	_delay_ms(2000);  // these 3 lines delay the uart output when plugged in in the hope of not being recognised as legacy mouse hardware  
	...
	ADCSRA |= 1<<ADPS2;		//adjust prescalar to division factor 16
	...
	uart0_init(1,0);  // Asyn,NoParity,1StopBit,8Bit,  -- baud rate: 115.2k
}
...
/*
* printStrip serially Prints with space padding for consistent positioning in terminal.
*/
void printStrip(int value)
{
	if(value < 10)
	{
		fprintf(&uart_str, "%d     ", value);
...
void getStrip_1_2(uint8_t sensor) {
	...
	digitalWrite(&PORTK, sideStrip[sensor][4], 1);
	setDir(&DDRK, sideStrip[sensor][2], 0);	
	digitalWrite(&PORTK, sideStrip[sensor][2], 0);  //gnd divider resistor
	setDir(&DDRK, sideStrip[sensor][3], 0);
	digitalWrite(&PORTK, sideStrip[sensor][3], 1); //sending 5V at both ends of the pot 
	setADC(sideStrip[sensor][5]);
	_delay_ms(1);
	int force = readADC();
	//position reading
	clear(sensor);
	setDir(&DDRK, sideStrip[sensor][3], 0); // disconnect reference divider resistor
	digitalWrite(&PORTK, sideStrip[sensor][3], 1);
	setDir(&DDRK, sideStrip[sensor][4], 0);
	digitalWrite(&PORTK, sideStrip[sensor][4], 0);
	setDir(&DDRK, sideStrip[sensor][2], 0);
	...
	digitalWrite(&PORTK, sideStrip[sensor][2], 0); //hiZ line 2/2
	setDir(&DDRK, sideStrip[sensor][1], 1);
	_delay_ms(1); //aid reading sensitivity
	int position = readADC();
//	if(force >= TOUCH_THRESH) { //With no force on the sensor the wiper is floating, position readings would be bogus
		fprintf(&uart_str, "FSLP #%d", sensor+1);
		fprintf(&uart_str, "'s Position: ");
		printStrip(position);
	 	fprintf(&uart_str, "Force: ");
	 	printStrip(force);
//	 } else {
	// 	fprintf(&uart_str, "No touch detected on FSLP#%d", sensor+1);
	// }
	 newline();
}
...
int main() {
	...
	while (1) {	
		...
		//for (uint8_t i = 0; i<4; i++) {  
			...
			//}
		//}
		_delay_ms(100);		//REMEMBER TO DELETE THIS FOR OPTIMISED FIRMWARE!
	}
	return 0;
}